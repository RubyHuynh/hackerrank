...
int divisibleSumPairs(int n, int k, int ar_count, int* ar) {
    ...
    for (i; i < ar_count; ++i) {
        completement = ar[i] % k;
        complementIdx = (k - completement) % k;
        printf ("\n===arr[%i] = %d\n", i, ar[i]);
        printf ("bucket_complement[%d] = %d\n", complementIdx, buckets[complementIdx]);
        rs += buckets[complementIdx];
        printf ("current rs = %d\n", rs);
        ++buckets[completement];
        printf ("bucket[%d] = %d\n", completement, buckets[completement]);
    }
    return rs;
}
